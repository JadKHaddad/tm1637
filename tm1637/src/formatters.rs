...
//!
//! Functions for converting numbers (i8s, i16s or i32s) into arrays of bytes
...
/// Formats two numbers between 0 and 99, with an optional colon between them.
///
...
pub fn f32_to_6digits(n:f32, decimals: u8) -> [u8;6] {
	...
	let mut m:i32 = ((n
		.mul(10i32.pow(decimals as u32) as f32)
		+ if n.is_sign_positive() { 0.5_f32 } else { -0.5_f32 })
		as i32)
		.clamp(-99999,999999)
		.abs();
